...
public class MDS {
	// Description given in readme.txt for the data structures that are taken for
	// storing the data
	...
	public int insert(long id, Money price, java.util.List<Long> list) {
		...
		if (!list.isEmpty()) { // Add the entries into newDesc only if list is not empty
			for (long listId : list) { // Iterating over the List entries to add it into newDesc(Hashset)
				...
				if (hSet == null) { // If the description is not present, then create a new entry in descIdMap with
				                    // the given description and id
					temp = new HashSet<>();
					temp.add(id);
					descIdMap.put(listId, temp);
				} else { // If the description is already present, add the id to the respective
				         // description in descIdMap
					hSet.add(id);
				}
			}
	...
	public int findPriceRange(long n, Money low, Money high) {
		...
		return sum; // Returning the total sum of items present in the given range w.r.to the given
		            // description
	}
	...
	public Money priceHike(long l, long h, double rate) {
		TreeSet<Long> hSet = (TreeSet<Long>) ids.subSet(l, true, h, true); // hSet contains the subset of ids between l and
		                                                                   // h
